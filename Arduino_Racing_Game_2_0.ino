 ...
 *    SW - D3
 *    
 *  Graphical models of the objects was taken from Konstantin Rozov youtube video. 
 *    Link: https://www.youtube.com/watch?v=VmgCsZFNaok&feature=emb_logo
 *    
 ...
 *    You should have received a copy of the GNU General Public License (COPYING.txt)
...
uint8_t b[8]={0x1f, 0x19, 0x15, 0x13, 0x19, 0x15, 0x13, 0x1f}; // It contains graphical model of the barrier. 
int Coords[16]; // It defines positions of the barrier objects for each column on the display (0 to 15), possible values: 0 - no barrier, 1 - barrier on the left side, 2 - barrier on the right side.
int last=0; // It must prevent situations in which player has no place to go, it means last value of Coords.
int last2=0; // It's here for the same reason, it means quantity of free space between barriers on different sides.
